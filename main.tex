...
Our goal is to find a solution $(\nabla,\Phi)\in \sA_{E}\times A^{1,0}(\End_{0}(E))$ to \Cref{eqn:hitchin} starting from a stable Higgs bundle $(E,\Phi)$.
This means in particular that we are given a holomorphic structure $\bar{\partial}_{E}$ on $E$ and a holomorphic Higgs field $\Phi\colon E\to E\ot K$.
The metric $h$ and the holomorphic structure $\bar{\partial}_{E}$ determine a unique element $\nabla_{0}\in \sA_{E}$ compatible with both of them, which we called the Chern connection.
We wish to modify $(\nabla_{0},\Phi)$ to produce a solution of \Cref{eqn:hitchin}, and to do this we have now two options:
\begin{itemize}
    \item Modify the holomorphic structure $\bar{\partial}_{E}$ and the Higgs field $\Phi$.
    \item Modify the Hermitian metric $h$.
\end{itemize}

The metric $h$ induces an affine-linear isomorphism from $\sA_{E}$ onto the space of holomorphic structures on $E$, and from now on we identify these two spaces under this isomorphism.
The \textit{complex gauge group} $\sG^{c}$, defined as the space $A^{0}(\Aut_{0}(E))$ of determinant $1$ automorphisms of $E$, acts on $\sA_{E}$ as follows:
\begin{itemize}
    \item Thinking of $\sA_{E}$ as the space
...
\[ \mu(\nabla)=F-2\pi i\mu(E)\1_{E}\w_{M}\in A^{2}(\u_{h}(E)), \]
