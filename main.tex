...
\begin{itemize}
    ...
    \item $\O$: structure sheaf.
    \item $K\to M$: canonical line bundle.
    \item $\w$: canonical sheaf.
    \item More generally, $O_{X}$, $\O_{X}$, $K_{X}$ and $\w_{X}$ denote the corresponding object on a complex manifold $X$.
...
\begin{prop}
	...
	Since $\O_{M}\cong p_{*}\O_{\P(E)}$, the projection formula implies that $p_{*}p^{*}(-)\cong (-)$ for locally free sheaves on $M$.
	The adjunction $p^{*}\dashv p_{*}$ induces then an isomorphism
	...
	We will show that the linear system $|p^{*}(\w\ot\Lambda^{2}\E)(2)|$ has dimension $2$ and has no fixed component, and then irreducibility follows from Bertini's theorem \cite[Theorem 7.19]{iit82}.
	...
	which implies that $h^{0}(\P(E),p^{*}(\w\ot \Lambda^{2}\E)(2))=h^{0}(M,\sEnd_{0}(\E)\ot \w)\geqslant 3$, as we wanted to show.

	So if $|p^{*}(\w\ot \Lambda^{2}\E)(2)|$ has no fixed component, then Bertini's theorem and the discussion above imply that a generic $\Phi\in H^{0}(M,\sEnd_{0}(\E)\ot \w)$ leaves no line bundle invariant, making $(E,\Phi)$ automatically stable.
	...
	By \cite[Exercise II.7.4]{har77} we can write $\M_{i}\cong p^{*}\L_{i}(l_{i})$ with $l_{1}+l_{2}=2$ and by \cite[Exercise III.8.4]{har77} we must have $0\leqslant l_{i}\leqslant 2$, because using again the adjunction $p^{*}\dashv p_{*}$ and the projection formula we have
