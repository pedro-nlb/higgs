...
Fix a Hermitian metric on $M$ normalized to unit volume, i.e.~such that
...
The difference between the two settings is the introduction of the Higgs field $\Phi$, so the first goal of this talk will be to understand stability of such pairs $(E,\Phi)$.
In the second part of the talk we will see how to modify Donaldson's proof to produce solutions to Hitchin's equations.
...
\begin{lm}\label{lm:traceless}
    For every $E\in \Vec_{2}(M)$ there is a short exact sequence
    ...
    \begin{proof}
	Over $x\in M$ we consider the projection $\End(E_{x})\to \End_{0}(E_{x})$ given by
	\[ \phi\mapsto \phi-\frac{\tr(\phi)}{2}\1_{E_{x}}. \]
	The endomorphisms in the kernel are precisely the multiples of the identity, and this globalizes to the desired short exact sequence.	
	\textcolor{gray}{%
	For the computation of the Euler characteristic we use \cite[Appendix A]{har77}.
	We have
...
\begin{lm}\label{lm:twocases}
	...
	Let $\Phi\in H^{0}(M,\sEnd_{0}(\E)\ot \sK)$.
	...
	Suppose that $\Phi\in H^{0}(M,\sEnd_{0}(\E)\ot \sK)$ is not nilpotent and let $L\subseteq E$ be a $\Phi$-invariant line bundle.
	...
	From \Cref{lm:traceless} we know that
...
\begin{cexe}{B}
	...
	$$\lambda^{2}-\alpha$$
	has a root in $H^{0}(M,K)$, i.e.~if and only if the quadratic differential $\alpha$ can be written as a square $\alpha=\beta^{2}$ for some differential $\beta\in H^{0}(M,K)$. If $\alpha$ was a square, its zeros would all have multiplicity at least two. Conclude that $K^{\frac{1}{2}}\op K^{-\frac{1}{2}}\in \bfA$ using Bertini's theorem.}]
\end{cexe}
...
\begin{prop}\label{prop:general}
	...
	So if $E\in \bfB$, then this $L$ must be the only line bundle invariant by all $\Phi\in H^{0}(M,\sEnd_{0}(\E)\ot \sK)$.
...
The plan is to reformulate \Cref{eqn:hitchin} in terms of a moment map and then apply the following:
...
\begin{metalm}\label{metalm}
	...
	\item The $G$ action admits a $G$-equivariant moment map
	    \[ \mu\colon N\to \g\dual. \]
	\item The Lie algebra $\g$ admits an adjoint-invariant inner product $\langle \cdot,\cdot \rangle$ under which we can identify $\g$ and $\g\dual$.
	\item The $G$ action extends to a complex group $G^{c}$ of holomorphic transformations.
    \end{itemize}
    Consider the smooth function $||\mu||^{2}\colon N\to \R$ given by
...
Let us see how to apply this to solve \Cref{eqn:hitchin}.
...
By \cite[(3.15)]{hit87a} from Jin's talk, the action of $G^{c}$ is free on the orbit of the stable pair $(\nabla_{0},\Phi_{0})$, so we can apply \Cref{metalm} to our situation.
Since the condition $\nabla^{0,1}\Phi$ is preserved by the $G^{c}$ action, it suffices to find a minimum of $||\mu||^{2}$ on the orbit of such a stable pair to produce a solution to \Cref{eqn:hitchin}.
...
But $\psi$ is non-zero and its determinant is a holomorphic function $M\to \C$, hence constant.
So we only have the following two possibilities:
\begin{itemize}
    \item $\psi$ is an isomorphism, yielding an element in $G^{c}$ that sends $(\nabla,\Phi)$ to $(\nabla_{0},\Phi_{0})$.
	In this case $(\nabla,\Phi)$ is in the same $G^{c}$-orbit as $(\nabla_{0},\Phi_{0})$, so we have found a solution to \Cref{eqn:hitchin}.
...
\textcolor{gray}{%
Indeed, let again $(\nabla_{n},\Phi_{n})$ be a minimizing sequence in the orbit of $(\nabla_{0},\Phi_{0})$, and assume now that $\Phi_{n}\in U$ for all $n\in \N$.
We may assume again that this sequence weakly converges to some $(\nabla,\Phi)$.
The equation $\mu=0$ is preserved under weak limits and, so $\mu(\nabla,\Phi)$ is a solution to \Cref{eqn:hitchin}, hence stable by \cite[(2.1]{hit87a}.
...
By \cite[(3.15)]{hit87a} from Jin's talk it must be an isomorphism, so $\psi$ yields an element in $G^{c}$ taking $(\nabla,\Phi)$ to $(\nabla_{0},\Phi_{0})$.
}
...
\begin{thm}
    ...
    Then there is an automorphism of $E$ with determinant $1$ which takes $(\nabla(h;\bar{\partial}_{E}),\Phi)$ to a solution of \Cref{eqn:hitchin}, unique up to gauge equivalence.
\end{thm}

We may think of this as an example of the Kempf--Ness theorem discussed in Vincent's talk, but in an infinite dimensional set up:

\begin{cor}[Hitchin--Kobayashi correspondence]
    Let $M$ be a compact Riemann surface of genus $g\geqslant 2$.
    Let $E$ be a complex vector bundle of rank $2$ on $M$ and $h$ a Hermitian metric on $E$.
    Let $G$ be the gauge group of special unitary automorphisms of $E$ and $G^{c}$ the complex gauge group of special linear automorphisms of $E$.
    Let $\calM^{s}//G^{c}$ be the moduli space of stable Higgs bundles, obtained as a GIT-quotient, and let $\calM//G$ be the moduli space of solutions to \Cref{eqn:hitchin}, obtained as a symplectic quotient.
    Then we have a bijection
    \[ \calM//G\cong \calM^{s}//G^{c}. \]
\end{cor}
