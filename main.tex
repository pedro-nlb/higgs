...
\begin{metalm}
	...
	\[ d\mu_{\xi}(-)=\w(\xi_{N},-). \]
	But since we are on a Kähler manifold we can also write
	\[ d\mu_{\xi}(-)=g(I\xi_{N},-). \]
...
The Lie algebra $\g=A^{0}(\u_{h}(E))$ of $G$ consists of skew-hermitian endomorphisms of $E$, and strictly speaking we have given an element in $A^{2}(\u_{h}(E))$ rather than in $\g\dual$.
But we can define on $\g$ an invariant inner product
\[ \langle\alpha,\beta\rangle:=\int_{M}\tr(\alpha\beta)\w_{M}, \]
and under the analogous pairing between $\g$ and $A^{2}(\u_{h}(E))$ we may identify $\mu(\nabla,\Phi)$ with an element in $\g\dual$.
...
Suppose that we manage to find a minimum $x_{0}:=\psi\cdot (\nabla_{0},\Phi_{0})$ along this orbit, so that the differential $df$ at $x_{0}$ vanishes on vector fields.
But since
\[ d(\langle \mu,\mu\rangle)=\langle d\mu,\mu\rangle+\langle \mu,d\mu\rangle =2\langle \mu, d\mu\rangle \]
...
This is always possible by definition of infimum, becuase our function $f$ takes values in $\R_{\geqslant 0}$.
