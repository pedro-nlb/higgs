...
Fix a Hermitian metric on $M$ such that
...
A solution to Hitchin's equations in this set up consists of a unitary connection $\nabla \colon A^{0}(E)\to A^{1}(E)$ and an endomorphism $\Phi\in A^{0}(\End(E)\ot K)$ twisted by the canonical line bundle $K\to M$ such that
\begin{equation}\label{eqn:hitchin}
    \begin{cases}
	F+[\Phi,\Phi^{*}]=-2\pi i \mu(E)\mathbbm{1}_{E}\w_{M}, \\
	\nabla^{0,1}\Phi=0.
    \end{cases}
\end{equation}
About the notation:
\begin{itemize}
    ...
    \item $\Phi$ can be locally written as $\phi dz$, where $\phi$ is a local section of $\End(E)$ and $z$ is a local holomorphic coordinate.
    ...
    \item $\mathbbm{1}_{E}\in A^{0}(\End(E))$ denotes the identity endomorphism.
...
The condition $\nabla^{0,1}\Phi=0$ then translates into $\Phi$ being a holomorphic section of $\End(E)\ot K$, which can then be regarded as a holomorphic morphism $E\to E\ot K$.
Moreover:

\begin{prop}
    Let $(E,\Phi)$ be an irreducible solution to \Cref{eqn:hitchin}, with $E$ an holomorphic vector bundle of rank $2$ and $\Phi\in H^{0}(M,\End(E)\ot K)$.
    Let $L\subseteq E$ be a holomorphic line subbundle such that $\Phi(L)\subseteq L\ot K$.
    If $\mu(L)\geqslant \mu(E)$, then the sequence $0\to L\to E \to E/L\to 0$ has a holomorphic splitting and we can write $\Phi=\Phi_{L}\op \Phi_{E/L}$.
    \begin{proof}
	The Hermitian metric gives us a orthogonal projection $P\in \Hom(E,L)\subseteq \End(E)$ such that $P^{2}=P$ and $P^{*}=P$, hence a smooth splitting $E\cong L\op L\prp$.
	From \cite[p.~141]{gh78} we have
	\[ \deg(L)=\frac{i}{2\pi}\int_{M}F_{L}, \]
	where $F_{L}$ is the curvature of the connection induced by $\nabla$ on $L$.
	The curvature decreases along holomorphic subbundles \cite[p.~79]{gh78}, and stays the same only if the splitting $E\cong L\op L\prp$ is holomorphic \cite[Proposition 1.6.14]{kob87}.
	By indecomposability, we deduce that
	

	Since $L$ is $\Phi$-invariant, we have $P\Phi P=\Phi P$.
	Using the cyclic property of the trace and the properties of $P$, it follows that $\tr(P[\Phi,\Phi^{*}])=|P\Phi(\mathbbm{1}_{E}-P)|^{2}$ \cite[(4.20)]{wen16}, where $|-|$ is the norm induced by the Hermitian product of complex matrices $\langle A,B\rangle=\tr(AB^{*})$.
    \end{proof}
\end{prop}
...
\begin{lm}\label{lm:stable}
	...
	So the previous short exact sequence is a non-trivial extension, hence its extension class $\xi$, defined as the image of $\id_{\sK}$ under the coboundary map $\delta\colon \Hom(\sK,\sK)\to \Ext^{1}(\sK,\O)$, is non-zero \cite[Exercise III.6.1]{har77}.
	\todo{finish: coboundary given by cup with extension class, then use duality}
	The coboundary map $H^{0}(M,\sK)\to H^{1}(M,\O)$ is non-zero.
...
Let us summarize what we have seen in this section in a single statement:

\begin{thm}
    Let $M$ be a compact Riemann surface of genus $g\geqslant 2$.
    Then
    \[ \Vec_{2}(M)=\bfA\sqcup \bfB, \]
    that is, for a rank $2$ vector bundle on $M$ we have two mutually exclusive possibilities:
    \begin{itemize}
	\item A general section $\Phi\in H^{0}(M,\sEnd(\E)\ot \sK)$ leaves no line bundle $L\subseteq E$ invariant.
	\item There exists a line bundle $L\subseteq E$ which is invariant under all sections $\Phi\in H^{0}(M,\sEnd(\E)\ot \sK)$.
    \end{itemize}
    Moreover, $E$ appears in a stable pair $(E,\Phi)$ if and only if a general section $\Phi\in H^{0}(M,\sEnd(\E)\ot\sK)$ leaves no line bundle $L\subseteq E$ invariant.
\end{thm}

Note in particular that if $E$ appears in a stable pair $(E,\Phi_{0})$, then $(E,\Phi)$ is trivially stable for a general $\Phi\in H^{0}(M,\sEnd(\E)\ot\sK)$.
