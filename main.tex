...
\begin{prop}[{\cite[Proposition 4.2.16]{wen16}}]
    Let $(E,\Phi)$ be a solution to \Cref{eqn:hitchin}, with $E$ an holomorphic vector bundle of rank $2$ and $\Phi\in H^{0}(M,\End(E)\ot K)$.
    Suppose we cannot write $E$ holomorphically as a direct sum of line bundles $L_{1}\op L_{2}$ such that $\Phi=\Phi_{1}\op \Phi_{2}$ for some $\Phi_{i}\in H^{0}(M,\End(L_{i})\ot K)$, $i\in \{1,2\}$.
    Then every holomorphic line subbundle $L\subseteq E$ such that $\Phi(L)\subseteq L\ot K$ verifies $\mu(L)<\mu(E)$. 
\end{prop}
