...
\begin{lm}\label{lm:stable}
	...
	Consider the short exact sequence from \Cref{lm:linearalgebra}
	\[ 0\to \E\dual\ot \L\ot\w\to \sEnd_{0}(\E)\ot \w \to \L^{-2}\ot \w\ot \Lambda^{2}\E\to 0. \]
	...
	more or less implicit in the proof of \Cref{lm:linearalgebra}.
...
Recall from \cite[\S 1]{hit87a} the \textit{self-duality equations}.
Let $M$ be a compact Riemann surface of genus $g\geqslant 0$ and let $P$ be a principal $\SO(3)$-bundle on $M$.
We want to find a connection $A$ on $P$ and a smooth $(1,0)$-form with values in the complexified adjoint vector bundle $\Phi\in \Omega^{1,0}(M,\ad(P)\ot \C)$ such that $d_{A}''\Phi=0$ and
\[ F(A)+[\Phi,\Phi^{*}]=0, \]
where $F(A)\in \Omega^{2}(M,\ad(P))$ is the curvature of the connection and
\[ [\Phi,\Phi^{*}](X,Y)=[\Phi(X),\Phi^{*}(Y)]+[\Phi^{*}(X),\Phi(Y)] \]
for all $X,Y\in \X(M)$.
After some reductions we end up considering a rank $2$ smooth complex vector bundle $E\to M$ with a fixed connection on $\Lambda^{2}E$ given by a projective embedding of $M$.
The complex vector bundle $\ad(P)\ot \C)$ is then the vector bundle of traceless endomorphisms of $E$ twisted by the canonical bundle $K$, hence $\Phi\in \Omega^{1,0}(M,\End_{0}(E)\ot K)$.
The condition that $d_{A}''\Phi=0$ says then that $\Phi$ is a holomorphic section with respect to the holomorphic structure induced by the connection $A$.

\begin{thm}
    In the previous set up, assume that $(E,\Phi)$ is a stable Higgs bundle.
    Then we can find an automorphism of $E$ with constant determinant $1$ which takes $(A,\Phi)$ to a solution of the equation $F(A)+[\Phi,\Phi^{*}]=0$.
    Moreover, this automorphism is unique up to gauge equivalence.
\end{thm}
